...
/**
 * @defgroup lifecycle_hook Hook-based Lifecycle Implementations
 * @brief Implementations that delegate to callback functions
 * @{
 */
...
/**
 * @brief Define a lifecycle method that delegates to a callback
 * @param Type The component type
 * @param stage The lifecycle stage
 */
#define eer_lifecycle_hook(Type, stage)                                        \
    void Type##_##stage(void *instance)                                        \
    {                                                                          \
        eer_self(Type, instance);                                              \
        if (self->on_##stage)                                                  \
...
/**
 * @brief Define the release lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_release_hook(Type)     eer_lifecycle_hook(Type, release)

/**
 * @brief Define the did_mount lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_did_mount_hook(Type)   eer_lifecycle_hook(Type, did_mount)

/**
 * @brief Define the did_update lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_did_update_hook(Type)  eer_lifecycle_hook(Type, did_update)

/**
 * @brief Define the did_unmount lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_did_unmount_hook(Type) eer_lifecycle_hook(Type, did_unmount)

/**
 * @brief Define the will_mount lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_will_mount_hook(Type)                                              \
    void Type##_will_mount(void *instance, void *next_props_ptr)               \
    {                                                                          \
        eer_self(Type, instance);                                              \
        if (!next_props_ptr)                                                   \
        ...
        if (&self->props != next_props)                                        \
            self->props = *next_props;                                         \
        if (self->on_will_mount)                                               \
            self->on_will_mount(self, next_props);                             \
    }

/**
 * @brief Define the will_update lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_will_update_hook(Type)                                             \
    void Type##_will_update(void *instance, void *next_props_ptr)              \
    {                                                                          \
        eer_self(Type, instance);                                              \
        if (!next_props_ptr)                                                   \
            next_props_ptr = &self->props;                                     \
        eer_selfnext(Type, instance);                                          \
        if (self->on_will_update)                                              \
            self->on_will_update(self, next_props);                            \
        if (&self->props != next_props)                                        \
            self->props = *next_props;                                         \
    }

/**
 * @brief Define the should_update lifecycle method that delegates to a callback
 * @param Type The component type
 */
#define eer_should_update_hook(Type)                                           \
    bool Type##_should_update(void *instance, void *next_props_ptr)            \
    {                                                                          \
        eer_self(Type, instance);                                              \
        if (!next_props_ptr)                                                   \
            next_props_ptr = &self->props;                                     \
        eer_selfnext(Type, instance);                                          \
        if (self->on_should_update)                                            \
            return self->on_should_update(self, next_props);                   \
        return true;                                                           \
    }
