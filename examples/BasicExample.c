...
DID_MOUNT(BasicComponent) {
  printf("BasicComponent mounted\n");
}

DID_UNMOUNT(BasicComponent) {
  printf("BasicComponent unmounted\n");
}

// Create a component instance with initial props
eer_withprops(BasicComponent, basicComponent, _({
  .value = 1
}));
...
int main() {
  ...
  loop(basicComponent) {
    ...
    apply(BasicComponent, basicComponent, _({
      .value = basicComponent.state.value + 1
    }));
